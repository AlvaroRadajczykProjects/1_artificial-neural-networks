...
def network_builder( input_layer, hidden_layers , output_layer ):
	...
	for i in range(len(nodes)-1):
		...
		#network[1].append(random.normal( loc=0, scale=2/nodes[i+1], size=( nodes[i], nodes[i+1] ) ).tolist())
...
def theorical_back_propagation( examples, network, learning_rate, fun_list, dfun_list, err_fun, derr_fun, show_error ):
	...
	for i in examples:
		...
		for j in range(len(calc[0])): utils.vector_mul(calc[0][j],learning_rate)
		for j in range(len(calc[1])): utils.matrix_mul(calc[1][j],learning_rate)
		#sub gradient vector to network
		for j in range(len(calc[0])): utils.vectors_add(network[0][j], calc[0][j], -1)
		for j in range(len(calc[1])): utils.matrixes_add(network[1][j], calc[1][j], -1)
